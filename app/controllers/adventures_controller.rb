class AdventuresController < ApplicationController
    ...
    def create
        ...
        if params[:adventure]["adventure_date"] != ""
            ...
            temp={"number_of_people"=>params[:adventure]["number_of_people"], "action_adventure"=>params[:adventure]["action_adventure"], "country"=>params[:adventure]["country"], "city"=>params[:adventure]["city"], "description"=>params[:adventure]["description"], "details"=>params[:adventure]["details"], "price"=>params[:adventure]["price"], "completed"=>params[:adventure]["completed"], "paid" => false}
        end
        ...
        if current_user.save
            ...
            if params[:adventure]["adventure_date"] != ""
                ...
                graph.put_object(553202955027429, "feed", {
                             :name => "Test post",
                             :message => "#{@adventure.user.first_name} from #{@adventure.user.city_of_residence} needs an #{@adventure.action_adventure} in #{@adventure.city},  #{@adventure.country} any time! #{@adventure.user.first_name} asked: \"#{@adventure.description} \". Eager to pay: $#{@adventure.price} for this #{@adventure.action_adventure}. https://ask-the-locals.herokuapp.com/adventures/" + "#{@adventure.id}"
